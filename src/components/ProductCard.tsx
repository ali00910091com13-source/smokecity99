...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <motion.div 
      ...
      whileHover={{ y: -10, boxShadow: '0 20px 40px rgba(0, 192, 127, 0.15)' }}
    ...
    >
      <div className="relative overflow-hidden aspect-square bg-gradient-to-br from-[#f8fafc] to-[#f1f5f9]">
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2">
          {product.isNew && (
            <motion.span 
              className="px-2 py-1 bg-[#00C07F] text-white text-xs font-bold rounded-lg"
          ...
          {product.isBestseller && (
            <motion.span 
              className="px-2 py-1 bg-[#F59E0B] text-white text-xs font-bold rounded-lg"
              initial={{ opacity: 0, scale: 0 }}
              animate={{ opacity: 1, scale: 1 }}
          ...
          {discount > 0 && (
            <motion.span 
              className="px-2 py-1 bg-[#EC4899] text-white text-xs font-bold rounded-lg"
              initial={{ opacity: 0, scale: 0 }}
              animate={{ opacity: 1, scale: 1 }}
        ...
        <motion.button
          ...
          className={`absolute top-3 left-3 w-9 h-9 rounded-full flex items-center justify-center ${
            liked ? 'bg-[#EC4899] text-white' : 'bg-white/80 text-[#6b7280] hover:bg-white hover:text-[#EC4899]'
          }`}
        ...
        <motion.button
          ...
          className="absolute bottom-3 right-3 w-10 h-10 bg-[#00C07F] text-white rounded-xl flex items-center justify-center opacity-0 group-hover:opacity-100 shadow-lg shadow-[#00C07F]/20"
      ...
      <motion.div 
        ...
        whileHover={{ backgroundColor: 'rgba(0, 192, 127, 0.05)' }}
      >
